...
function checkRankUp() {
    ...
    // Wizualny efekt awansu (opcjonalnie)
    if (state.xp % 1000 < 20) vibrate([100, 50, 100]);
}
...
function calculateDaily() {
    ...
    const subs = state.subscriptions.reduce((s, b) => s + b.amount, 0);
    
    return ((state.income - state.fixed - subs) - spent) / days;
}

// Zero-Day Clock: Na ile dni starczy Ci kasy przy obecnym tempie?
...
function toggleDecoy() {
    ...
    vibrate(200);
    updateUI();
    alert(state.isDecoy ? "DECOY ACTIVE" : "SHADOW PROTOCOL RESTORED");
}

/* --- INTERFEJS I AKTUALIZACJA --- */
function updateUI() {
    const daily = calculateDaily();
    const amountEl = document.getElementById('daily-amount');
    
    // Jeśli Decoy jest aktywny, pokazujemy fałszywe dane
    if (state.isDecoy) {
        ...
        if (daily < 20) {
            document.getElementById('stashek-talk').innerText = dialogues.bad;
        } else {
            document.getElementById('stashek-talk').innerText = dialogues.good;
        }
    }
...
function initGame() {
    ...
    gameInterval = setInterval(() => {
        ...
        // Rysowanie Staszka (Twój icon.png)
        const img = document.getElementById('stashek-mood');
        ctx.drawImage(img, szopX - 30, canvas.height - 80, 60, 60);

        if(Math.random() < 0.05) {
            const types = [
                {i: '🪙', v: 5, c: '#ffd700'}, // Moneta
                {i: '💵', v: 20, c: '#00ff88'}, // Banknot
                {i: '☕', v: -15, c: '#ff8c00'}, // Kawa (ujemna)
                {i: '🧾', v: -50, c: '#ff4444'}  // Rachunek (mocno ujemny)
            ];
        ...
        items.forEach((item, i) => {
            ...
            if(item.y > canvas.height - 90 && Math.abs(item.x - szopX) < 40) {
                score += item.v;
                addXP(item.v > 0 ? 2 : 0); // XP tylko za dobre rzeczy
...
// Ukryta funkcja: 5 szybkich kliknięć w zegar aktywuje Decoy Mode
let clockClicks = 0;
